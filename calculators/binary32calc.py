import os
import cffi
from softfloat import float2bits32
...
class Binary32Calc(binarycalc.BinaryCalc):
    def _float_calculator(self):
        lhs = float2bits32(self.first_num)
        rhs = float2bits32(self.second_num)
    ...
    def __sum(self, lhs, rhs):
        res = lib.f32_add(lhs, rhs)
        return res

    def __multiply(self, lhs, rhs):
        res = lib.f32_mul(lhs, rhs)
        return res

    def __div(self, lhs, rhs):
        res = lib.f32_div(lhs, rhs)
        return res
